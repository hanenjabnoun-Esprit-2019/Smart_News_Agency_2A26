...
employe:: employe (int id , QString nom ,QString prenom ,QString role , int salaire ,QString email , int numero , int id_dep ){
...
{
    QSqlQuery q ;
    q.prepare("insert into employe values (:id , :nom ,:prenom , :role , :salaire ,:email , :numero ,:id_dep )");
    q.bindValue(":id",id);
        q.bindValue(":nom",nom);
...
{
    QSqlQuery q ;
    q.prepare("update employe set nom=:nom ,prenom=:prenom , role=:role , salaire=:salaire ,email=:email , numero=:numero where id=:id");
            q.bindValue(":id",id);
                q.bindValue(":nom",nom);
                    q.bindValue(":prenom",prenom);
                        q.bindValue(":role",role);
